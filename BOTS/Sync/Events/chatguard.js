...
module.exports = async (message) => {
    function guvenli(kisiID) {
      ...
      if (config.kufurengel === "true") {
        if (message.author.bot || !message.guild) return;
             ...
             if (mateasbadword.some(mateasbadword => ` ${message.content.toLowerCase()} `.includes(` ${mateasbadword} `))) { 
                let chatkullanım = db.get(`ChatKullanım.${message.member.id}`)
                if (chatkullanım >= 10) { 
                  message.channel.send(`!mute ${message.member} 5m Chat'i fazla kirletmek`)
                  db.delete(`ChatKullanım.${message.member.id}`, 10) 
                  
                }
                if (!chatkullanım) return db.add(`ChatKullanım.${message.member.id}`, 1) 
                db.add(`ChatKullanım.${message.member.id}`, 1)
                if(message.deletable) message.delete({timeout: 0020}).catch(console.error); return message.reply(`Küfür etmen yasak lütfen küfür içerikli mesaj atmamaya dikkat et.Chat'i fazla kirletirsen ceza alıcaksın kalan uyarı sayısı : \`${10 - chatkullanım || '10'}\``)
        ...
        if (message.member.hasPermission('ADMINISTRATOR') || guvenli(message.member.id)) return;
            ...
            const reklamfiltre = ["discord.gg","discord.com/invite","discordapp.com/invite","https://"]
               if (reklamfiltre.some(word => message.content.includes(word))) {
                ...
                if (!chatkullanım) return db.add(`ChatKullanım.${message.member.id}`, 1) 
                db.add(`ChatKullanım.${message.member.id}`, 1)
                if(message.deletable) message.delete({timeout: 0010}).catch(console.error); return message.reply(`Reklam yapman yasak lütfen reklam yapmamaya dikkat et.Chat'i fazla kirletirsen ceza alıcaksın kalan uyarı sayısı : \`${10 - chatkullanım || '10'}\``).then(a => a.delete({timeout: 5000}))}
                else if (config.reklamengel == 'false') {
    }}


    if (config.mentionengel === "true") {
        if (message.author.bot || !message.guild) return;
           if (message.member.hasPermission('ADMINISTRATOR') || guvenli(message.member.id)) return;
          
            if (message.mentions.users.size >= 6) { 
                 message.channel.send(`!mute ${message.member.id} 5m Çok fazla kişi etiketlemek.`);
              if(message.deletable) message.delete({timeout: 0030}).catch(console.error);
            }
            else if (config.mentionengel == 'false') {
            }
        }
        ...
        if (config.capsengel === "true") {
            ...
            return message.channel.send(`${message.author}, Lütfen daha çok küçük harf kullanmaya özen göster.Chat'i fazla kirletirsen ceza alıcaksın kalan uyarı sayısı : \`${10 - chatkullanım || '10'}\``).then(a => a.delete({timeout: 5000}))
            }
            }}
  ...
  if(message.content.length > "500") {
    ...
    return message.channel.send(`${message.member} Lütfen mesajınızı 500 karakterden az yazınız.Chat'i fazla kirletirsen ceza alıcaksın kalan uyarı sayısı : \`${10 - chatkullanım || '10'}\``).then(a => a.delete({timeout: 5000}))
  }}
