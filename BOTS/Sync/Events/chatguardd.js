...
module.exports = async (oldMessage, newMessage) => {
    ...
    if (config.kufurengel === "true") {
        if (newMessage.author.bot || !newMessage.guild) return;
        if (newMessage.member.hasPermission('ADMINISTRATOR') || guvenli(newMessage.member.id)) return;
          
              if (mateasbadword.some(mateas => ` ${newMessage.content.toLowerCase()} `.includes(` ${mateas} `))) {
                let chatkullanım = db.get(`ChatKullanım.${newMessage.member.id}`)
                if (chatkullanım >= 10) { 
                  newMessage.channel.send(`!mute ${newMessage.member} 5m Chat'i fazla kirletmek`)
                  db.delete(`ChatKullanım.${newMessage.member.id}`, 10) 
                  
                }
                if (!chatkullanım) return db.add(`ChatKullanım.${message.member.id}`, 1) 
                db.add(`ChatKullanım.${newMessage.member.id}`, 1)
                if(newMessage.deletable) newMessage.delete({timeout: 0020}).catch(console.error); return newMessage.reply(`Küfür etmen yasak lütfen küfür içerikli mesaj atmamaya dikkat et.Chat'i fazla kirletirsen ceza alıcaksın kalan uyarı sayısı : \`${10 - chatkullanım || '10'}\``).then(a => a.delete({timeout: 5000}))}
      
      else if (config.kufurengel == 'false') {
    }}

    if (config.capsengel === "true") {
        if(newMessage.author.bot || !newMessage.guild) return;
        if(newMessage.member.hasPermission('ADMINISTRATOR') || guvenli(newMessage.member.id)) return;
        ...
        return newMessage.channel.send(`${newMessage.author}, Lütfen daha çok küçük harf kullanmaya özen göster.Chat'i fazla kirletirsen ceza alıcaksın kalan uyarı sayısı : \`${10 - chatkullanım || '10'}\``).then(a => a.delete({timeout: 5000}))
        }
         }}
     else if (config.capsengel == 'false') {
     }
    }

   if (config.morewordengel === "true") {
    if(newMessage.author.bot || !newMessage.guild) return;
    if(newMessage.member.hasPermission('ADMINISTRATOR') || guvenli(newMessage.member.id)) return;
    ...
    if(newMessage.content.length >= "500") {
     let chatkullanım = db.get(`ChatKullanım.${newMessage.member.id}`)
      ...
      if (!chatkullanım) return db.add(`ChatKullanım.${newMessage.member.id}`, 1) 
      db.add(`ChatKullanım.${newMessage.member.id}`, 1)
      if(newMessage.deletable)  newMessage.delete({timeout: 0050}).catch(console.error); 
      return newMessage.channel.send(`${newMessage.member} Lütfen mesajınızı 500 karakterden az yazınız.Chat'i fazla kirletirsen ceza alıcaksın kalan uyarı sayısı : \`${10 - chatkullanım || '10'}\``).then(a => a.delete({timeout: 5000}))
    }}
    else if (config.capsengel == 'false') {
    }


    if (config.reklamengel === "true") {
      if (newMessage.author.bot || !newMessage.guild) return;
      if (newMessage.author.bot || !newMessage.guild) return;
      if (newMessage.member.hasPermission('ADMINISTRATOR') || guvenli(newMessage.member.id)) return;
              ...
              if (reklamfiltre.some(word => newMessage.content.includes(word))) {
                ...
                if (!chatkullanım) return db.add(`ChatKullanım.${newMessage.member.id}`, 1) 
                db.add(`ChatKullanım.${newMessage.member.id}`, 1)
                if(newMessage.deletable) newMessage.delete({timeout: 0010}).catch(console.error); return newMessage.reply(`Reklam yapman yasak lütfen reklam yapmamaya dikkat et !Chat'i fazla kirletirsen ceza alıcaksın kalan uyarı sayısı : \`${10 - chatkullanım || '10'}\``).then(a => a.delete({timeout: 5000}))}
          else if (config.reklamengel == 'false') {
    }}
    else if (config.capsengel == 'false') {
    }
}
