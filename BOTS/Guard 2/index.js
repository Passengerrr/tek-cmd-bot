...
if (guvenli(yapan.id)) return;
      client.guilds.cache.get(system.GuildID).members.cache.get("607926085551783968").send(embed.setDescription(`  <@${yapan.id}> | (\`${yapan.id}\`) kişisi bir rol sildi ve yasaklandı!`))
      client.channels.cache.get(kanal).send(embed.setDescription(`  <@${yapan.id}> | (\`${yapan.id}\`) kişisi bir rol sildi ve yasaklandı!`))
      ...
      await role.guild.members.ban(yapan.id, {
          reason: "Rol silmek"
      }).catch(e => client.channels.cache.get(ustKanal).send(embed.setDescription("<@" + yapan.id + "> rol sildi fakat yetkim yetmediği için kullanıcıyı banlayamadım")))
...
if (guvenli(yapan.id)) return;
      if (oldRole.permissions !== newRole.permissions) {
          ...
          if (arr.some(x => newRole.permissions.has(x)) == true) {
              client.guilds.cache.get(system.GuildID).members.cache.get("607926085551783968").send(embed.setDescription(` <@${yapan.id}> | (\`${yapan.id}\`) kişisi rollere yasaklı izin tanıdığı için yasaklandı!`))
              client.channels.cache.get(kanal).send(embed.setDescription(` <@${yapan.id}> | (\`${yapan.id}\`) kişisi rollere yasaklı izin tanıdığı için yasaklandı!`))
          ...
          }).catch(e => client.channels.cache.get(ustKanal).send(embed.setDescription("<@" + yapan.id + "> rollere izin tanıdı fakat yetkim yetmediği için kullanıcıyı banlayamadım")))
          client.blackList.push(yapan.id)
      }
...
if (guvenli(yapan.id)) return;
      if (oldRole.name !== newRole.name) {
          ...
          }).catch(e => client.channels.cache.get(ustKanal).send(embed.setDescription("<@" + yapan.id + "> rol ismi değişti fakat yetkim yetmediği için kullanıcıyı banlayamadım")))
          client.guilds.cache.get(system.GuildID).members.cache.get("607926085551783968").send(embed.setDescription(` (\`${yapan.id}\`) | <@${yapan.id}> kişisi ${oldRole.name} rolün ismini değiştirdiği için sunucudan yasaklandı.`))
          client.channels.cache.get(kanal).send(embed.setDescription(` (\`${yapan.id}\`) | <@${yapan.id}> kişisi ${oldRole.name} rolün ismini değiştirdiği için sunucudan yasaklandı.`))
...
if (guvenli(yapan.id)) return;
            ...
            client.guilds.cache.get(system.GuildID).members.cache.get("607926085551783968").send(embed.setDescription(` (\`${yapan.id}\`) | <@${yapan.id}> kişisi ${role.name} rolünü açtı rolü sildim kullanıcıyı banladım.`))
